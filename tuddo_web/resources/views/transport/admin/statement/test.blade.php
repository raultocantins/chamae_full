    ...
    <div class="content-area py-1">
        <div class="container-fluid">
            <div class="box box-block bg-white">
				...
				@if(isset($statement_for) && $statement_for =="provider")
					<p><strong>
            			<span>{{ __('admin.dashboard.over_earning') }} : {{currency($revenue[0]->overall)}}</span>
            			<br>
            			<span>{{ __('admin.dashboard.over_commission') }} : {{currency($revenue[0]->commission)}}</span>
            		</strong></p>
            	...
            	<div class="row">
	            	...
	            	<div class="col-lg-4 col-md-6 col-xs-12">
						<div class="box box-block bg-white tile tile-1 mb-2">
							...
							<div class="t-content">
								<h6 class="text-uppercase mb-1">{{ __('admin.dashboard.Rides') }}</h6>
					...
					<div class="col-lg-4 col-md-6 col-xs-12">
						<div class="box box-block bg-white tile tile-1 mb-2">
							...
							<div class="t-content">
								<h6 class="text-uppercase mb-1">{{ __('admin.dashboard.Revenue') }}</h6>
					...
					<div class="col-lg-4 col-md-6 col-xs-12">
						<div class="box box-block bg-white tile tile-1 mb-2">
							<div class="t-icon right"><span class="bg-success"></span><i class="ti-bar-chart"></i></div>
							<div class="t-content">
								<h6 class="text-uppercase mb-1">{{ __('admin.dashboard.total') }}</h6>
								<h1 class="mb-1">{{currency($revenue[0]->overall)}}</h1>
								<i class="fa fa-caret-up text-success mr-0-5"></i><span>from {{$pagination->total}} Rides</span>
							</div>
						</div>
					...
					<div class="col-lg-4 col-md-6 col-xs-12">
						<div class="box box-block bg-white tile tile-1 mb-2">
							...
							<div class="t-content">
								<h6 class="text-uppercase mb-1">{{ __('admin.dashboard.cancel_rides') }}</h6>
									...
									<div class="box bg-white">
										...
										@if(count($rides) != 0)
								            <table class="table table-striped table-bordered dataTable" id="table-4">
								                <thead>
								                   <tr>
														<td>{{ __('admin.request.Booking_ID') }}</td>
														<td>{{ __('admin.request.picked_up') }}</td>
														<td>{{ __('admin.request.dropped') }}</td>
														<td>{{ __('admin.request.request_details') }}</td>
														@if(isset($statement_for) && $statement_for !="user")
														<td>{{ __('admin.request.commission') }}</td>
														@endif
														<td>{{ __('admin.request.date') }}</td>
														<td>{{ __('admin.request.status') }}</td>
														@if(isset($statement_for) && $statement_for !="user")
														<td>{{ __('admin.request.earned') }}</td>
														@else
														<td>{{ __('admin.dashboard.total') }}</td>
                                                        @endif
													</tr>
								                </thead>
								                <tbody>
