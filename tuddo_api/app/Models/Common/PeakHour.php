...
class PeakHour extends BaseModel
{
    ...
    public function getStartedTimeAttribute() {
        ...
        $timezone=isset($this->attributes['timezone']) ? $this->attributes['timezone'] : config('constants.timezone', 'America/Sao_Paulo');
        return (isset($this->attributes['start_time'])) ? (Carbon::createFromFormat('H:i:s', $this->attributes['start_time'], config('constants.timezone', 'America/Sao_Paulo')))->setTimezone($timezone)->format('H:i:s') : '' ;
    }

    public function getEndedTimeAttribute() {
        
        $timezone=isset($this->attributes['timezone']) ? $this->attributes['timezone'] : config('constants.timezone', 'America/Sao_Paulo');
        return (isset($this->attributes['end_time'])) ? (Carbon::createFromFormat('H:i:s', $this->attributes['end_time'], config('constants.timezone', 'America/Sao_Paulo')))->setTimezone($timezone)->format('H:i:s') : '';
    }
