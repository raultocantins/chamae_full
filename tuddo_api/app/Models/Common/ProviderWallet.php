...
class ProviderWallet extends BaseModel
{
    ...
    public function getCreatedTimeAttribute() {
        $timezone=isset(Auth::guard('provider')->user()->state_id) ? State::find(Auth::guard('provider')->user()->state_id)->timezone : config('constants.timezone', 'America/Sao_Paulo');
        return (isset($this->attributes['created_at'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['created_at'], config('constants.timezone', 'America/Sao_Paulo')))->setTimezone($timezone)->format('Y-m-d H:i:s') : '' ;
