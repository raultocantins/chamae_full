...
class Ride {
	...
	*/
	...
	public function createRide(Request $request) {
		...
		$timezone = State::find($this->user->state_id)->timezone; // (Auth::guard('user')->user()->state_id) ? State::find($this->user->state_id)->timezone : '';
		...
		if($request->has('schedule_date') && $request->schedule_date != "" && $request->has('schedule_time') && $request->schedule_time != ""){
			...
			$schedule_date = (Carbon::createFromFormat('Y-m-d H:i:s', (Carbon::parse($request->schedule_date. ' ' .$request->schedule_time)->format('Y-m-d H:i:s')), $timezone))->setTimezone(config('constants.timezone', 'America/Sao_Paulo'));
		...
		try {
			...
			if($request->has('schedule_date') && $request->schedule_date != "" && $request->has('schedule_time') && $request->schedule_time != ""){
				...
				$rideRequest->schedule_at = (Carbon::createFromFormat('Y-m-d H:i:s', (Carbon::parse($request->schedule_date. ' ' .$request->schedule_time)->format('Y-m-d H:i:s')), $timezone))->setTimezone(config('constants.timezone', 'America/Sao_Paulo'));
			...
			return ['message' => ($rideRequest->status == 'SCHEDULED') ? trans('api.ride.schedule_request_created') : trans('api.ride.new_request_created'), 'data' => [
						'message' => ($rideRequest->status == 'SCHEDULED') ? trans('api.ride.schedule_request_created') : trans('api.ride.new_request_created'),
	...
	public function updateRide(Request $request) {
		try{
			...
			if($request->status == 'DROPPED' && ($rideRequest->payment_mode != 'CASH' && $rideRequest->payment_mode != 'MACHINE')) {
				$rideRequest->status = 'COMPLETED';
				$rideRequest->paid = 0;

				(new SendPushNotification)->Complete($rideRequest, 'transport');
			} else if ($request->status == 'COMPLETED' && ($rideRequest->payment_mode == 'CASH' || $rideRequest->payment_mode == 'MACHINE')) {
				...
				$RequestPayment->payment_mode = $rideRequest->payment_mode;
			...
			if($rideRequest->use_wallet == 1 && $payable_amount > 0){
				...
				if($Wallet != 0){
					...
					if($payable_amount > $Wallet) {
						...
						if($rideRequest->payment_mode == 'CASH' || $rideRequest->payment_mode == 'MACHINE'){
