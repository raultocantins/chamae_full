...
class CustomPushController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->has('page') && $request->page == 'all') {
            $Pushes = CustomPush::where('company_id',Auth::user()->company_id)->get();
        } else {
	...
	public function store(Request $request){
		...
		$this->validate($request, [
                ...
                'search_radius_user' => 'required_if:user_condition,LOCATION',
                ...
                'search_radius_provider' => 'required_if:provider_condition,LOCATION',
		...
		try{
			...
			if($request->send_to == 'USERS'){
				...
				if($request->user_condition == 'ACTIVE'){
					...
					$CustomPush->condition_data = "" . ($request->search_radius_user > 0 ? $request->search_radius_user : 10) . "," . $request->user_location;
				...
				if($request->provider_condition == 'ACTIVE'){
					...
					$CustomPush->condition_data = "" . ($request->search_radius_provider > 0 ? $request->search_radius_provider : 10) . "," . $request->provider_location;
        ...
        try{
            ...
            if($Push->send_to == 'USERS'){
                ...
                if($Push->condition == 'ACTIVE'){
                    ...
                    $Users = User::whereRaw("round((6371 * acos( cos( radians(" . $latitude . ") ) * cos( radians(latitude) ) * cos( radians(longitude) - radians(" . $longitude . ") ) + sin( radians(" . $latitude . ") ) * sin( radians(latitude) ) ) ),3) <= $distance")
                            ->pluck('id','id')->toArray();
