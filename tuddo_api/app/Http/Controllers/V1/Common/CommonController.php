...
class CommonController extends Controller
{
    ...
    public function cmspagetype($type) {
        $data = CmsPage::where('page_name', $type)->get();
    ...
    public function chat(Request $request) 
    {
        ...
        if($user_request != null) {
            ...
            return Helper::getResponse(['message' => trans('api.user.inserted') ]);
        } else {
    ...
    public function sendOtp(Request $request) {
        ...
        if(Helper::setting()->site->send_sms == 1){
            $send_otp = Helper::send_sms($company_id, '+'.$request->country_code.''.$request->mobile, 'Your OTP is ' . $otp . '. Do not share your OTP with anyone' );
        }else if(Helper::setting()->site->send_email == 1){
            $subject='OTP';
            $templateFile='mails/registermailotp';
            $data=['body'=>$otp,'username'=>$request->name, 'salt_key' => $company_id];

            $send_otp= Helper::send_emails($templateFile,$request->mail,$subject, $data); 
        }else{
            return Helper::getResponse(['status' => '400', 'message' => 'Could not send OTP notification. Please try again!', 'error' => "555"]);
        }
        
        if($send_otp === true) {
            return Helper::getResponse(['message' => 'OTP sent!']);
        } else {
            return Helper::getResponse(['status' => '400', 'message' => 'Could not send OTP notification. Please try again!', 'error' => $send_otp]);
        }
    }
