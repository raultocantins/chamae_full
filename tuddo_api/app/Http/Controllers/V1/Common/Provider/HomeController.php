...
class HomeController extends Controller
{
	...
	public function show_profile()
	{
		try {
			...
			if ($settings->site->referral == 1) {
				...
				$provider_details['referral']->referral_amount = (float)($settings->site->referral_amount);
	...
	public function update_location(Request $request)
	{
		...
		try {
			...
			return Helper::getResponse(['message' => trans('api.user.updated')]);
		} catch (\Throwable $e) {
			return Helper::getResponse(['status' => 500, 'message' => $e->getMessage()]);
		}
	}
	...
	public function walletlist(Request $request)
	{
		if ($request->has('limit')) {
			$provider_wallet = ProviderWallet::select('id', 'transaction_id', 'transaction_alias', DB::raw('SUM(amount) as amount'), 'transaction_desc', 'type', 'created_at')->with(['transactions', 'payment_log' => function ($query) {
				$query->select('id', 'company_id', 'is_wallet', 'user_type', 'payment_mode', 'user_id', 'transaction_code');
			}])
			...
			$provider_wallet = ProviderWallet::select('id', 'provider_id', 'transaction_id', 'transaction_alias', DB::raw('SUM(amount) as amount'), 'transaction_desc', 'type', 'created_at')
				->with(['transactions', 'payment_log' => function ($query) {
