...
class ProviderAuthController extends Controller
{
	...
	public function login(Request $request) {
		...
		if($User->activation_status == 0){
            return Helper::getResponse(['status' => 422, 'message' => trans('api.account_disabled')]);
        }
	...
	public function signup(Request $request) {
		...
		$Admin = Admin::where([['country_id', $request->country_id], ['city_id', $request->city_id], ['company_id', base64_decode($request->salt_key)]])->first();
	...
	public function document_store(Request $request) {
		...
		try {
			...
			foreach ($request->file('file') as $image) {
				...
				$file = Helper::upload_providerfile($image, 'provider/documents', $file_name, Auth::guard('provider')->user()->company_id);
	...
	public function logout(Request $request) {
		try {
			...
			return Helper::getResponse(['message' => trans('api.user.logged_out')]);
