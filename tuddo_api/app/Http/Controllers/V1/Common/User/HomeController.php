...
class HomeController extends Controller
{
    ...
    public function addmanageaddress(Request $request)
    {
        ...
        try {
            ...
            $uAddress = UserAddress::where('user_id', Auth::guard('user')->user()->id)->get();
    ...
    public function updatedefaultaddress(Request $request)
    {
        ...
        try {
            ...
            if($useraddress){
                UserAddress::where('company_id', Auth::guard('user')->user()->company_id)->where('user_id', Auth::guard('user')->user()->id)->update(['default' => 0]);

                $useraddress = UserAddress::where('id', $request->id)->update(['default' => 1]);
    ...
    public function deletemanageaddress($id)
    {
        try {
            ...
            if($uAddress->count() == 1){
                $ua = UserAddress::findOrFail($uAddress[0]->id);
                $ua->default = 1;
                $ua->save();
            }
    ...
    public function addcard(Request $request)
    {
        ...
        try {
            ...
            $card = $customer->createSource($customer_id, ["source" => $request->stripe_token]);
    ...
    public function walletlist(Request $request)
    {
        if ($request->has('limit')) {
            $user_wallet = UserWallet::select('id', 'transaction_id', 'transaction_desc', 'transaction_alias', 'type', 'amount', 'created_at')->with(['payment_log' => function ($query) {
                $query->select('id', 'company_id', 'is_wallet', 'user_type', 'payment_mode', 'user_id', 'amount', 'transaction_code');
            }])
            ...
            $user_wallet = UserWallet::select('id', 'user_id', 'transaction_id', 'transaction_alias', 'transaction_desc', 'type', 'amount', 'created_at')->with(['payment_log' => function ($query) {
                $query->select('id', 'company_id', 'is_wallet', 'user_type', 'payment_mode', 'user_id', 'amount', 'transaction_code');
            }, 'user' => function ($query) {
    ...
    public function countries(Request $request)
    {
        ...
        $country_list = CompanyCountry::with(['companyCountryCities' => function ($q) use ($company_id) {
            $q->where('company_id', $company_id);
        }])->has('companyCountryCities')->where('company_id', $company_id)->where('status', 1)->get();
        $countries = [];
    ...
    public function promocode(Request $request)
    {
        ...
        $promocode = Promocode::with('service')->whereHas('service', function ($query) {
            $query->where('status', 1);
        })->where('company_id', \Auth::guard('user')->user()->company_id)->whereDate('expiration', '>=', Carbon::today())->orderby('id', 'desc')->get();
        ...
        try {
            ...
            if (count($notifications) > 0) {
                foreach ($notifications as $k => $val) {
                    $notifications[$k]['created_at'] = (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $val['created_at'], config('constants.timezone', 'America/Sao_Paulo')))->setTimezone($timezone)->format('Y-m-d H:i:s');
                }
            }
