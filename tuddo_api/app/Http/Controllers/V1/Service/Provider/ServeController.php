...
class ServeController extends Controller 
{
	...
	public function updateServe(Request $request)
	{
		...
		try{
			...
			if($request->status == 'PAYMENT' && ($serveRequest->payment_mode != 'CASH' && $serveRequest->payment_mode != 'MACHINE')) {
				$serveRequest->status = 'COMPLETED';
				$serveRequest->paid = 0;

				(new SendPushNotification)->serviceProviderComplete($serveRequest, 'service', 'Service Completed');
			} else if ($request->status == 'PAYMENT' && ($serveRequest->payment_mode == 'CASH' || $serveRequest->payment_mode == 'MACHINE')) {
				...
				$RequestPayment->payment_mode = $serveRequest->payment_mode;
            ...
            if($UserRequest->use_wallet == 1 && $Total > 0){
                ...
                if($Wallet != 0 && $Wallet != 0.00 ){
                    if($Payamount > $Wallet){
						...
						if($UserRequest->payment_mode == 'CASH' || $UserRequest->payment_mode == 'MACHINE'){
	...
	public function rate(Request $request)
    {
		...
		try {
			...
			if($serviceRequest != null){
				...
				if((($paymode == 'CASH' || $paymode == 'MACHINE') && $requestStatus == 'COMPLETED') || (($paymode != 'CASH' && $paymode != 'MACHINE') && $requestStatus == 'DROPPED')|| (($paymode != 'CASH' && $paymode != 'MACHINE') && $requestStatus == 'COMPLETED'))
				{
	...
	public function getServiceRequestDispute(Request $request,$id) {
		$service_request_dispute = ServiceRequestDispute::where('company_id',Auth::guard('provider')->user()->company_id)
								...
								->where('dispute_type','provider')
								->first();
		$service_request_dispute->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $service_request_dispute->created_at, config('constants.timezone', 'America/Sao_Paulo')))->setTimezone(Auth::guard('provider')->user()->timezone)->format(Helper::dateFormat()); 						
	...
	public function callTransaction($request_id){
		...
		if($UserRequest->paid==1){
			...
			if($paymentsRequest->is_partial==1){
				//partial payment
				if($paymentsRequest->payment_mode == "CASH" || $paymentsRequest->payment_mode == "MACHINE"){
