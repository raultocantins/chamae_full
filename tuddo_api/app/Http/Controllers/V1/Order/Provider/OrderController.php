...
class OrderController extends Controller
{
	...
	public function updateOrderStaus(Request $request)
	{
		...
		try{
			...
			if($request->status == 'PAYMENT' && ($serveRequest->orderInvoice->payment_mode != 'CASH' && $serveRequest->orderInvoice->payment_mode != 'MACHINE')) {
				$serveRequest->status = 'COMPLETED';
				// $serveRequest->paid = 0;

				(new SendPushNotification)->orderProviderComplete($serveRequest, 'order', 'Order Completed');
			} else if ($request->status == 'PAYMENT' && ($serveRequest->orderInvoice->payment_mode == 'CASH' || $serveRequest->orderInvoice->payment_mode == 'MACHINE')) {
				...
				if($siteConfig->cashback == "1" && $siteConfig->cashback_percentage > 0 && $RequestPayment->wallet_amount == 0){
					$cachevalue = $RequestPayment->total * ( $siteConfig->cashback_percentage / 100 );
					...
					$transaction_alias= 'CHS'.str_pad($transaction_id, 6, 0, STR_PAD_LEFT);

					$ipdata=array();
					$ipdata['company_id'] = Auth::guard('provider')->user()->company_id;
					$ipdata['transaction_id'] = $transaction_id;
					$ipdata['transaction_alias'] = $transaction_alias;
					$ipdata['transaction_desc'] = "Cashback";
					$ipdata['id'] = $serveRequest->user->id;        
					$ipdata['type'] = "C";
					$ipdata['amount'] = $cachevalue;
					(new Transactions)->createUserWallet($ipdata);
				}
	...
	public function getOrderRequestDispute(Request $request,$id) {
		$order_request_dispute = StoreOrderDispute::where('company_id',Auth::guard('provider')->user()->company_id)
								...
								->where('dispute_type','provider')
								->first();
			$order_request_dispute->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $order_request_dispute->created_at, config('constants.timezone', 'America/Sao_Paulo')))->setTimezone(Auth::guard('provider')->user()->timezone)->format(Helper::dateFormat()); 					
