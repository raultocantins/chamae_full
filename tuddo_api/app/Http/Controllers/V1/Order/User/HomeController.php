...
class HomeController extends Controller
{
    ...
    public function store_list(Request $request, $id)
    {
        ...
        if ($request->has('filter') && $request->filter != '') {
            $store_list_all->whereHas('StoreCusinie', function ($q) use ($request) {
                $q->whereIn('cuisines_id', explode(',', $request->filter));
            });
        }
    ...
    {
        $cusine_list = Cuisine::where('company_id', $this->company_id)->where('store_type_id', $id)
            ->get();
    ...
    public function store_details(Request $request, $id)
    {
        ...
        $store_details = Store::with([
            ...
            'products.itemsaddon', 'products.itemsaddon.addon', 'products.itemcart' => function ($query) use ($request) {
                $query->where('user_id', $this->user->id);
            }
        ...
        $store_detail->totalcartprice = number_format( $this->totalusercart()->sum('total_item_price'), 2, '.', '');
    ...
    public function shoptime($id)
    {
        ...
        if (!empty($shop_timing)) {
            ...
            $timezone = isset($state_id->state_id) ? State::find($state_id->state_id)->timezone : config('constants.timezone', 'America/Sao_Paulo');

            $key = key($shop_timing);
            $current_time = \Carbon\Carbon::now($timezone);
            $start_time = (Carbon::createFromFormat('H:i', (Carbon::parse($key)->format('H:i')), $timezone))->setTimezone(config('constants.timezone', 'America/Sao_Paulo'));
            //$start_time = \Carbon\Carbon::parse($key);
            $end_time = (Carbon::createFromFormat('H:i', (Carbon::parse($shop_timing[$key])->format('H:i')), $timezone))->setTimezone(config('constants.timezone', 'America/Sao_Paulo'));
    ...
    public function viewcart(Request $request)
    {
        ...
        try {
            ...
            if (!$CartItems->isEmpty()) {
                ...
                $cityprice = StoreCityPrice::where('store_id', $CartItems[0]->store->id)->first();

                if ($cityprice) {
                    $UserAddress = UserAddress::where('user_id', $CartItems[0]->user_id)->where('default', 1)->first();

                    $deliveryCharge = $cityprice->delivery_charge;

                    if($UserAddress){
                        $user_latitude = $UserAddress->latitude;
                        $user_longitude = $UserAddress->longitude;

                        $store_latitude = $CartItems[0]->store->latitude;
                        $store_longitude = $CartItems[0]->store->longitude;

                        $apiurl = "https://maps.googleapis.com/maps/api/distancematrix/json?origins=".$store_latitude.",".$store_longitude."&destinations=".$user_latitude.",".$user_longitude."&mode=driving&sensor=false&units=metric&key=".$this->settings->site->server_key;
                        ...
                        $distance_km = round( $meters / 1000);

                        $delivery_charge_minkm = $cityprice->delivery_charge_minkm;
                        $delivery_charge_kmmulti = $cityprice->delivery_charge_kmmulti;

                        if($delivery_charge_minkm > 0 && $delivery_charge_kmmulti > 0){
                            if($distance_km > $delivery_charge_minkm){
                                $extra_km = $distance_km - $delivery_charge_minkm;
                                if($extra_km > 0){
                                    $deliveryCharge = $cityprice->delivery_charge + ($delivery_charge_kmmulti * $extra_km);
                                }
                            }
                        }

                        // $teste = [
                        //     'current_delivery_charve' => $cityprice->delivery_charge,
                        //     'delivery_charge_minkm' => $delivery_charge_minkm,
                        //     'delivery_charge_kmmulti' => $delivery_charge_kmmulti,
                        //     'distance_km' => $distance_km,
                        //     'extra_km' => ($distance_km - $delivery_charge_minkm),
                        //     'total_deliveryCharge' => $deliveryCharge
                        // ];
                        // \Log::info($teste);
                    }
                } else {
                ...
                $total_net = ($net + $store_tax + $deliveryCharge + $store_package_charge);
    ...
    public function totalusercart()
    {
        ...
        $CartItems = StoreCart::select('total_item_price')->where('company_id', $this->company_id)->where('user_id', $this->user->id)->get();
        return $CartItems;
    }
    ...
    public function checkout(Request $request)
    {
        ...
        $order->timezone = State::find(Auth::guard('user')->user()->state_id)->timezone; // (Auth::guard('user')->user()->state_id) ? State::find(Auth::guard('user')->user()->state_id)->timezone : '';
        ...
        if ($order->id) {
            ...
            $orderinvoice->delivery_amount = ($order->order_type == "TAKEWAY" ? 0 : $cart['delivery_charges']);
    ...
    public function shoprequestHistory(Request $request)
    {
        try {
            ...
            if ($request->has('limit') && $request->has('type')) {
                $data = $datum->where("status", $request->type)->take($request->limit)->offset($request->offset)->get();
            } else {
    ...
    public function get_order_request_dispute(Request $request, $id)
    {
        ...
        if ($order_request_dispute) {
            $order_request_dispute->created_time = (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $order_request_dispute->created_at, config('constants.timezone', 'America/Sao_Paulo')))->setTimezone($order_request_dispute->request->timezone)->format(Helper::dateFormat());
        }
