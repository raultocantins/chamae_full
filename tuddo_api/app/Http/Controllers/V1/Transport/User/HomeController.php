...
use Actions;
	...
	public function get_ride_request_dispute(Request $request,$id) {
		...
		$ride_request_dispute->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ride_request_dispute->created_at, config('constants.timezone', 'America/Sao_Paulo')))->setTimezone($ride_request_dispute->request->timezone)->format('d-m-Y g:i A');
		}							
	...
	public function get_ride_lost_item(Request $request,$id) {
		$ride_lost_item = RideLostItem::where('company_id',Auth::guard('user')->user()->company_id)
								->where('ride_request_id',$id)
								->first();
		$ride_lost_item->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ride_lost_item->created_at, config('constants.timezone', 'America/Sao_Paulo')))->setTimezone(Auth::guard('user')->user()->timezone)->format(Helper::dateFormat());					
