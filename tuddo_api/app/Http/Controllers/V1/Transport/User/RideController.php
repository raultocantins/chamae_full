...
class RideController extends Controller
{
	...
	public function services(Request $request)
	{
		...
		$ride_delivery_vehicles = RideDeliveryVehicle::with(['priceDetails' => function($q) {
			$q->where('city_id', $this->user->city_id);
	...
	public function estimate(Request $request)
	{
		...
		$request->request->add(['city_id' => $this->user->city_id]);
		...
		return Helper::getResponse(['data' => ['fare' => $fare, 'service' => $service, 'promocodes' => $promocodes, 'unit' => $this->settings->transport->unit_measurement, 'currency' => Auth::guard('user')->user()->currency_symbol ]]);
	}
	...
	public function update_payment_method(Request $request)
	{
		...
		try{
			...
			if($request->payment_mode != "CASH" && $request->payment_mode != "MACHINE") {
