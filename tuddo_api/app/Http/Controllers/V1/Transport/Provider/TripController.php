...
class TripController extends Controller
{
	...
	public function get_ride_request_dispute(Request $request,$id) {
		$ride_request_dispute = RideRequestDispute::where('company_id',Auth::guard('provider')->user()->company_id)
								...
								->where('dispute_type','provider')
								->first();
	    $ride_request_dispute->created_time=(\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $ride_request_dispute->created_at, config('constants.timezone', 'America/Sao_Paulo')))->setTimezone(Auth::guard('provider')->user()->timezone)->format(Helper::dateFormat());								
